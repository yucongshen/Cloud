...
<body>
<form class="form-inline definewidth m20" action="<%=basePath%>process/findByKeyPidAndUid?userId=${userId}" method="post">
	进程号：
    <input type="text" name="pid" id="pid" value="${pid}" class="abc input-default" placeholder="" >&nbsp;&nbsp; 
    <button type="submit" class="btn btn-primary nav-item-inner nav-order">查询</button>&nbsp;&nbsp;
</form>
<form  id="uploadForm" action="<%=basePath%>process/upload?userId=${userId}" method="post" enctype="multipart/form-data">
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <c:forEach items="${processList}" var="process" varStatus="status">
    		<tr>
    			...
    			<td><a href="<%=basePath%>processManage/findByPid?pid=${process.pid}">查看</a></td>
    		</tr>
    </c:forEach>
    </tbody>
</table>
